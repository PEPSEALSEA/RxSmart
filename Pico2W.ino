...
const String FIREBASE_RTDB_URL = "https://rxsmart-rehab-2026-default-rtdb.asia-southeast1.firebasedatabase.app"; // Firebase Realtime Database URL
...
bool writeMPU6050Register(uint8_t address, uint8_t reg, uint8_t value) {
  ...
  activeWire->write(reg);
  activeWire->write(value);
...
bool readMPU6050(
  ...
  int readBytes = activeWire->requestFrom((int)address, 14);
  if (readBytes != 14) {
    return false;
  }
  ...
  activeWire->read();
  activeWire->read();
  gx = (activeWire->read() << 8) | activeWire->read();
  gy = (activeWire->read() << 8) | activeWire->read();
  gz = (activeWire->read() << 8) | activeWire->read();
  return true;
}
